...
describe("isTwitterImage", () => {
  it("returns true for Twitter media URLs with image format", () => {
    expect(
      isTwitterImage("https://pbs.twimg.com/media/Glfh8q2awAA8nXq?format=png&name=small"),
    ).toBe(true)
  })
  ...
  it("returns false for non-Twitter URLs", () => {
    expect(isTwitterImage("https://example.com/media/img?format=png")).toBe(false)
  })

  it("returns false for Twitter URLs without media path", () => {
    expect(isTwitterImage("https://pbs.twimg.com/profile/abc?format=png")).toBe(false)
  })

  it("returns false for unsupported format", () => {
    expect(isTwitterImage("https://pbs.twimg.com/media/abc?format=mp4&name=orig")).toBe(false)
  })
})
...
describe("upgradeTwitterImageUrl", () => {
  it("sets name=orig for Twitter media URLs", () => {
    expect(upgradeTwitterImageUrl("https://pbs.twimg.com/media/abc123?format=jpg&name=small")).toBe(
      "https://pbs.twimg.com/media/abc123?format=jpg&name=orig",
    )
  })

  it("adds name=orig when name param is missing", () => {
    expect(upgradeTwitterImageUrl("https://pbs.twimg.com/media/abc123?format=jpg")).toBe(
      "https://pbs.twimg.com/media/abc123?format=jpg&name=orig",
    )
  })
...
describe("isImageURL", () => {
  ...
  it("returns true for Twitter image URLs", () => {
    expect(isImageURL("https://pbs.twimg.com/media/abc?format=jpg&name=orig")).toBe(true)
  })
...
describe("getFileName", () => {
  it("extracts filename from a simple URL", () => {
    expect(getFileName("https://example.com/images/photo.png")).toBe("photo.png")
  })

  it("extracts filename with query parameters", () => {
    expect(getFileName("https://example.com/photo.jpg?w=100")).toBe("photo.jpg")
  })

  it("adds extension for Twitter image URLs", () => {
    expect(getFileName("https://pbs.twimg.com/media/Glfh8q2awAA8nXq?format=png&name=small")).toBe(
      "Glfh8q2awAA8nXq.png",
    )
  })
  ...
  it("extracts filename from a Pixiv original image URL", () => {
    expect(
      getFileName("https://i.pximg.net/img-original/img/2024/01/01/00/00/00/12345678_p0.png"),
